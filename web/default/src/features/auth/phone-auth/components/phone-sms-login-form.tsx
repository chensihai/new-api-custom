...
export function PhoneSmsLoginForm({
  className,
  redirectTo,
  onSwitchToPassword,
  ...
  const form = useForm<z.infer<typeof phoneSmsLoginFormSchema>>({
    resolver: zodResolver(phoneSmsLoginFormSchema),
    defaultValues: {
      phone: '',
      code: '',
    },
  ...
  async function onSubmit(data: z.infer<typeof phoneSmsLoginFormSchema>) {
    ...
    try {
      ...
      if (res.success) {
        if ((res.data as Record<string, unknown>)?.require_2fa) {
          redirectTo2FA()
          return
        }
